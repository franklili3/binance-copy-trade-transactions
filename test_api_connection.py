...
def test_connection():
    ...
    logger.info(f"Secret Key: {'*' * 10 if secret_key else 'None'}")
    ...
    try:
        ...
        logger.info(f"交易所URL: {exchange.urls['api']['public']}")
        logger.info(f"沙盒模式: {exchange.sandbox}")
        ...
        try:
            ...
            logger.info(f"✓ 交易历史获取成功，共 {len(trades)} 条记录")
        except Exception as e:
            logger.warning(f"✗ 交易历史获取失败: {e}")
        
        # 测试4: 获取订单历史
        logger.info("测试4: 获取订单历史...")
        try:
            orders = exchange.fetch_orders('BTC/USDT', limit=1)
            logger.info(f"✓ 订单历史获取成功，共 {len(orders)} 条记录")
        except Exception as e:
            logger.warning(f"✗ 订单历史获取失败: {e}")
